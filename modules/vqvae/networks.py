...
class Upsample(nn.Module):
    def __init__(self, input_nc, output_nc, kernel, stride):
        super(Upsample, self).__init__()
        self.up = nn.ConvTranspose2d(input_nc, output_nc, kernel, stride)
    def forward(self, x):
        return self.up(x)
...
class Downsample(nn.Module):
    def __init__(self, input_nc, output_nc, kernel, stride):
        ...
        self.down = nn.Conv2d(input_nc, output_nc, kernel, stride)
    def forward(self, x):
        return self.down(x)
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], num_kps = 10):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        net = ResnetGeneratorNoDownSample(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnet_6blocks':
        net = ResnetGeneratorNoDownSample(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netG == 'resnet_18blocks':
        net = ResnetGeneratorNoDownSample(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=18)
...
def kp2gaussian(kp, spatial_size, kp_variance='single'):
    """
...
class ResnetGeneratorNoDownSample(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetGeneratorNoDownSample, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
